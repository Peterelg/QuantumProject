...
def save_data(P, Computer_Name, circuit_Size, gap_size, graph_size, output):
    now = datetime.now()
    name = "n" + str(P) + "c" + Computer_Name + "circuit" + str(circuit_Size) + "time" + str(now) + "gap" + str(gap_size) + "graph" + str(graph_size) + '.csv'
    ...
    with open(name, 'w') as csv_file:
        ...
        for i in range(length):
            data = [output['Results'][i]['a'], output['Results'][i]['b'], output['Results'][i]['Valid'], output['Results'][i]['Percentage of results']]
